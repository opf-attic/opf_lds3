...
if (!$show_content) {
        ...
        echo 'Simple Document Submission Test';
        echo '</title></head>';
        echo '<body>';
...
#       if (strpos($base_url,"amazon") != false) {
#               $stuff = getFakeContent($iriPrefix);
#       }
        ...
        for ($i=0;$i<count($http_response_header);$i++) {
                ...
                if (strtolower(substr($header,0,strlen("link"))) == "link") {
                        $parts = explode(":",$header,2);
                        $editiri = trim($parts[1]);
                        if (strpos($editiri,"edit-iri") > 0) {
                                  $editiri = substr($editiri,strpos($editiri,'<')+1,strlen($editiri));
                                  $editiri = substr($editiri,0,strpos($editiri,'>'));
                                  $return["editiri"] = true;
                        }
                }
...
function getFakeContent($iriPrefix) {
        if (substr($iriPrefix,-1) == "/") {
                $iriPrefix = substr($iriPrefix,0,strlen($iriPrefix)-1);
        }
        $content = '
...
<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
  ...
  <rdf:Description rdf:about="http://s3.amazonaws.com/training1/test.rdf">
...
function is_docurl_annotated($stuff,$docurl) {
	...
	include_once('libraries/arc2/ARC2.php');
        ...
        $amount = count(explode(", ",$relation_list));
        if ($amount > 0) {
                return true;
        }
        return false;
